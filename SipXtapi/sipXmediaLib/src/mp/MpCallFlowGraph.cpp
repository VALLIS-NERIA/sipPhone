      ...
      int recorderMask)
{
   ...
   if (recorderMask & 1)
      sprintf_s(created_micNamePtr, MAXUNIXPATH,
   ...
   if (recorderMask & 2)
      sprintf_s(created_echoOutNamePtr, MAXUNIXPATH,
   ...
   if (recorderMask & 4)
      sprintf_s(created_spkrNamePtr, MAXUNIXPATH,
   ...
   if (recorderMask & 8)
      sprintf_s(created_mic32NamePtr, MAXUNIXPATH,
   ...
   if (recorderMask & 16)
      sprintf_s(created_spkr32NamePtr, MAXUNIXPATH,
   ...
   if (recorderMask & 32)
      sprintf_s(created_echoIn8NamePtr, MAXUNIXPATH,
   ...
   if (recorderMask & 64)
      sprintf_s(created_echoIn32NamePtr, MAXUNIXPATH,
...

{
   ...
   if (NULL != audioFileName) {
        file = _open(audioFileName, O_BINARY | O_CREAT | O_RDWR, 0600);
   }
...
UtlBoolean MpCallFlowGraph::writeWAVHeader(int handle)
{
    ...
    bytesWritten += _write(handle,tmpbuf, strlen(tmpbuf));
    bytesWritten += _write(handle, (char*)&length, sizeof(length)); //filled in on close
    
    //write WAVE & length
    //8 bytes written
    strcpy(tmpbuf,"WAVE");
    bytesWritten += _write(handle,tmpbuf, strlen(tmpbuf));
//    bytesWritten += _write(handle,&length, sizeof(length)); //filled in on close
    ...
    //16 bytes written
    bytesWritten += _write(handle, (char*)&compressionCode, sizeof(compressionCode));
    bytesWritten += _write(handle, (char*)&numChannels, sizeof(numChannels));
    bytesWritten += _write(handle, (char*)&samplesPerSecond, sizeof(samplesPerSecond));
    bytesWritten += _write(handle, (char*)&averageSamplePerSec, sizeof(averageSamplePerSec));
    bytesWritten += _write(handle, (char*)&blockAlign, sizeof(blockAlign));
    bytesWritten += _write(handle, (char*)&bitsPerSample, sizeof(bitsPerSample));
