...
*/
   ...
   if (JbQueueSize == JbQCount + numSamples) //Make sure we have at least 1 packet of headroom to avoid memory conflicts.
   { 
      // discard some data...
      JbQOut = JbQOut + numSamples;
   ...
   {
       assert(JbQIn = JbQueueSize); /* If we are over queue size, haven't we overrun the queue? */
       JbQIn -= JbQueueSize;
   }
   return 0;
}
...
int MpJitterBuffer::GetSamples(Sample *voiceSamples, JB_size *pLength)
{
    ...
    if (JbQWait == 0 )
    {
      ...
      //{
      //  
      //  printf("pJitterBuffer::GetSamples:  JbQCount=%d, JbQWait=%d, JbQOut=%d, JbQIn=%d\n", JbQCount, JbQWait, JbQOut, JbQIn);
      //}
      if (JbQCount < numSamples)
      {
          //assert (JbQWait == JbLatencyInit); //We want to know if we have been into this twice.
          //printf("pJitterBuffer::GetSamples < 2 frames.  JbQCount=%d, JbQWait=%d, JbQOut=%d, JbQIn=%d\n", JbQCount, JbQWait, JbQOut, JbQIn);
          //fflush( stdout );
          JbQWait = JbLatencyInit; // No data, prime the buffer (again).Z
      ...
      else
      {
          ...
          if (JbQOut >= JbQueueSize) 
          {
              assert(JbQOut = JbQueueSize); /* If we are over queue size, haven't we overrun the queue? */
              JbQOut -= JbQueueSize;
          }
      }
    }
