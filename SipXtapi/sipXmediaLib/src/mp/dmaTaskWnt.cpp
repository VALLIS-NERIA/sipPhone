...
struct DSoundEnumContext {
  int AudioDriverEnumIndex;
  DSoundDevice deviceList[MAX_AUDIO_DEVICES];
};
#endif
...
#ifdef USE_DIRECTX
//Calbck procedure to enumerate DirectX sound devices
BOOL CALLBACK DSoundEnumCallback( GUID* pGUID, LPSTR strDesc, LPSTR strDrvName,
                                  VOID* pContext )
{
    DSoundEnumContext *context = (DSoundEnumContext*) pContext;
    if(context->AudioDriverEnumIndex < MAX_AUDIO_DEVICES)
    {
      if (pGUID)
      {
          context->deviceList[context->AudioDriverEnumIndex].deviceGuid = *pGUID;
      } else {
          context->deviceList[context->AudioDriverEnumIndex].deviceGuid = GUID_NULL;
      }
      context->deviceList[context->AudioDriverEnumIndex++].deviceName = strDesc;
    }
    return TRUE;
}

GUID getDeviceGuid(UtlString deviceName, bool isCapture)
{
  GUID rv = GUID_NULL;
  DSoundEnumContext context;
  for (int i = 0; i < MAX_AUDIO_DEVICES; i++)
  {
    context.deviceList[i].deviceGuid = GUID_NULL;
    context.deviceList[i].deviceName = "";
  }
  context.AudioDriverEnumIndex = 0;
  if (isCapture){
    DirectSoundCaptureEnumerate((LPDSENUMCALLBACK)DSoundEnumCallback, &context);
  } else {
    DirectSoundEnumerate((LPDSENUMCALLBACK)DSoundEnumCallback, &context);
  }
    for (int i = 0; i < context.AudioDriverEnumIndex; i++)
    {
      if (context.deviceList[i].deviceName == deviceName)
      {
        rv = context.deviceList[i].deviceGuid;
        break;
      }
    }
  return rv;
...
int DmaTask_setSpkrQPreload(int qlen)
{
   ...
   OsSysLog::add(FAC_MP, PRI_NOTICE, " Changing spkr preload from %d to %d frames\n", save, qlen);
...
{    
    ...
    // If using DirectX the speaker thread must be started first.  It shouldn't
    // matter otherwise.
