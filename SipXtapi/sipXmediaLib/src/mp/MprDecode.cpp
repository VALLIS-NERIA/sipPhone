...
static int numWarnings = 0;
   ...
   else
   {
      ...
      int pt;
      int dejitterPackets;
      unsigned int dejitterLowTimestamp;
      pDej->getPacketsInfo(dejitterPackets, dejitterLowTimestamp);
      
      if (dejitterPackets > mPreloading) {
      //while (NULL != (rtp = pDej->pullPacket())) {
         ...
         if (NULL != pCurDec) {
            ...
            mSkipFrames = rtp->numSamples / samplesPerFrame - 1;
            if (packetLen > 0) {
      ...
      if (dejitterPackets == 0) {
        mPreloading = MIN_RTP_PACKETS;
      }
      //if ((dejitterPackets < 2) || (dejitterPackets > 14))
      //{
      //  printf("dejitterPackets = %d timmer = %d\n", dejitterPackets, pOsTC);
      //  fflush( stdout);
      //}
   }
