...
OsStatus MprDejitter::pushPacket(MpBufPtr pRtp)
{
   ...
   MpBuf_touch(pRtp);
   mRtpLock.acquire();
   seqNum = getSeqNum(pRtp);
   // Check to see if this seqNum has recently been skipped
   if (!(mPacketsPulled &&
      (mLastSeqNumPulled - seqNum >= 0) &&
      (mLastSeqNumPulled - seqNum < MAX_RTP_PACKETS)))
   {
     mSeqNumDropCounter = 0;
     index = seqNum % MAX_RTP_PACKETS;
   ...
   else if (mPacketsPulled) // This sequence number has already been skipped, discard the stale packet.
   {
     assert(false);
     mNumDiscarded++;
     mSeqNumDropCounter++;
     if (mSeqNumDropCounter > MAX_RTP_PACKETS) {  /* the computer has fallen out of sync, reset the buffer */
       mPacketsPulled = false;
       mSeqNumDropCounter = 0;
...
MpBufPtr MprDejitter::pullPacket(void)
{
   ...
   mRtpLock.acquire();
   if ((mpPackets[(mLastSeqNumPulled + 1) % MAX_RTP_PACKETS] != NULL) &&
       (getSeqNum(mpPackets[(mLastSeqNumPulled + 1) % MAX_RTP_PACKETS]) == mLastSeqNumPulled + 1))
   {
     first = (mLastSeqNumPulled + 1) % MAX_RTP_PACKETS;
     firstSeq = getSeqNum(mpPackets[(mLastSeqNumPulled + 1) % MAX_RTP_PACKETS]);
   }
   else
...
OsStatus MprDejitter::getPacketsInfo(int& nPackets, unsigned int& lowTimestamp)
{
  nPackets = mNumPackets;             
  lowTimestamp = 0; //Todo: Calculate and return timing information for decoder
  return OS_SUCCESS;
}
