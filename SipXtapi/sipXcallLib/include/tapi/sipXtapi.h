...
/**
 * Set recorderMask for sipxAudioCallRecordFileStart.  
 *
 * @param recorderMask is a bitmapped integer to determine which components of
 *        the call to record.  Each is recorded into a seperate file.
 *       
 *        bitmask of recorderMask is as follows:
 ...
 *                        +----------------------------- not used
 */
SIPXTAPI_API SIPX_RESULT sipxCallAudioRecordFileSetMask(int RecorderMask) ;

/**
 * Record a call session (including other parties if this is a multi-party 
 * call / conference) to a file.  The resulting file will be a .raw file
 * with L16 encoding (8000 samples/per second).
 *
 * @param hCall Handle to a call.  Call handles are obtained either by 
 *        invoking sipxCallCreate or passed to your application through
 *        a listener interface.  Audio files can only be played and stopped
