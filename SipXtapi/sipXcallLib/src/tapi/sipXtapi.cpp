...
// GLOBAL VARIABLES
static int gRecorderMask = 5;  //default to Recording Spkr and Mic
                                                      ...
                                                      const char* szFile)
{
    ...
    {        
        if (pInst->pCallManager->audioChannelRecordStart(callId, remoteAddress, szFile, gRecorderMask))
        {
            sr = SIPX_RESULT_SUCCESS;
        }
    }
                ...
                (iLevel <= VOLUME_MAX))
        {
                    ...
                    pInst->speakerSettings[type].iVol != iLevel)
            {
                ...
                if (pInst->enabledSpeaker == type)
                {
                    ...
                    assert(status == OS_SUCCESS) ;
                    pInst->speakerSettings[type].iVol = iVolume;
//There seems to be some variation  assert(iVolume == iLevel) ;
                    if (status != OS_SUCCESS)
                    {
                        sr = SIPX_RESULT_FAILURE ;
                    }
...
//                assert(pInst->speakerSettings[type].bInitialized) ;
//            }
//            pInst->speakerSettings[type].iVol = 
